...
class Plot_Methods:
    ...
    def FuncObTime(self, t, Xt, Y, Xa, name):
        x_step = [10]
        x_start = [0]
        x_end = [100]
        x_label = "time(day)"
        ...
        for j in range(len(x_step)):
            plt.figure()
            plt.xticks(np.arange(x_start[j], x_end[j], step=x_step[j]))
            plt.plot(t[x_start[j]*4:x_end[j]*4],
                     Xt[1, x_start[j]*4:x_end[j]*4], label=line_labels[0])
            plt.plot(t[x_start[j]*4:x_end[j]*4],
                     Y[1, x_start[j]*4:x_end[j]*4], label=line_labels[1])
            plt.plot(t[x_start[j]*4:x_end[j]*4],
                     Xa[1, x_start[j]*4:x_end[j]*4], label=line_labels[2])
            plt.legend()
            plt.grid(color='k', linestyle='dotted', linewidth=0.5)
            plt.xlim(x_start[j], x_end[j]-1)
            plt.xlabel(x_label)
    ...
    def AnalysisRMSEandTrace(self, t, RMSE, Trace, name):
        ...
        for j in range(len(x_step)):
            ...
            plt.xticks(np.arange(x_start[j], x_end[j], step=x_step[j]))
            plt.plot(t[x_start[j]*4:x_end[j]*4],
                     RMSE[x_start[j]*4:x_end[j]*4], label="Xa_RMSE")
            plt.plot(t[x_start[j]*4:x_end[j]*4],
                     Trace[x_start[j]*4:x_end[j]*4], label="Pa_Trace")
            plt.legend()
            plt.grid(color='k', linestyle='dotted', linewidth=0.5)
            plt.xlim(x_start[j], x_end[j]-1)
            plt.xlabel(x_label)
            ...
            plt.title(title)
            plt.savefig(
                self.path + "AnalysisRMSEandTrace/AnalysisRMSEandTrace-" + name + ".png")
            plt.close()
    ...
    def AnalysisErrCovariance(self, Pa):
        ...
        for k in range(len(day)):
            fig = plt.figure()
            plt.xticks(np.arange(x_start[j], x_end[j], step=x_step[j]))
            sc = plt.imshow(Pa[:, :, day[k]*4], cmap=cmap,
            ...
            plt.title(title)
            plt.savefig(
                self.path + "AnalysisErrCovariance/AnalysisErrCovariance-delta-day-"+str(day[k]) + ".png")
            plt.close()
    ...
    def Debug(self, data, name):
        ...
        n = RANGE/5
        bounds = [-RANGE, -n*4, -n*3, -n*2, -
                  n, -0.1, 0.1, n, n*2, n*3, n*4, RANGE]
